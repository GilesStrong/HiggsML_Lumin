...
class Result():
    ...
    def load_exps(self) -> None:
        self.exps = []
        for m in self.machines:
            p = Path(self.path/f'{self.exp_name}_{m}.json')
            if p.exists():  self.exps.append(Experiment.from_json(p))
...
class ExpComp():
    ...
    def lookup_metric(metric:str) -> None:
        if metric == 'val_ams_smooth':        return 'Val. AMS at cut (MVAC)'
        if metric == 'val_ams_max':           return 'Maximum Val. AMS (MMVA)'
        if metric == 'test_public_ams_mean':  return 'Mean Public AMS (MAPA)'
...
def get_bottleneck_output(model, bottleneck_idx:int, inputs, pre_act:bool=False) -> np.ndarray:
    bn = model.body.bottleneck_blocks[bottleneck_idx]
    if pre_act:
        hook = FowardHook(bn[0])
        model.predict(inputs)
        return to_np(bn[0](hook.input[0]).squeeze())
    else:
